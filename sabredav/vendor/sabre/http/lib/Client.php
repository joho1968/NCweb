...
class Client extends EventEmitter {
    ...
    function send(RequestInterface $request) {
        ...
        do {
            ...
            try {
                ...
                // error_log ('>> '.__FUNCTION__ . ': getStatus');
                ...
                // error_log ('>> '.__FUNCTION__ . ': exception');
    ...
    protected function doRequest(RequestInterface $request) {
        ...
        // error_log ('>> '.__FUNCTION__ . ': curlExec');
    ...
    protected function createCurlSettingsArray(RequestInterface $request) {
        ...
        // error_log ('-- '.__FUNCTION__ . ': curlOptions URL='.$request->getUrl ());
    ...
    protected function curlExec($curlHandle) {
        ...
        curl_setopt ($this->curlHandle, CURLOPT_STDERR, $fp);
        error_log ('>>> '.__FUNCTION__ . ' DOING EXEC');
        $rc = curl_exec($curlHandle);
        error_log ('>>> '.__FUNCTION__ . ' DONE EXEC');
        fclose($fp);
        $debug = ob_get_clean();
        error_log (__FUNCTION__ . ":\n".$debug."\n---END---\n");
        error_log ('<<< '.__FUNCTION__);
